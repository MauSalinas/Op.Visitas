...
$idrendicion=$_SESSION['rendicion']; //ejemplo para leer restriccion 
...
if (isset($_POST) && count($_POST)>0)
{
	...
	else
	{
		...
		$query=$db->query("update Table set ".$_POST["campo"]."='".$_POST["valor"]."' where id='".intval($_POST["id"])."' limit 1");
...
if (isset($_GET) && count($_GET)>0)
{
	...
	else
	{
		...
		$query=$db->query("select * from Table  where Columna='$rest' order by columna");
                $datos=array();
